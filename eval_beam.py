"""
Evaluate NOVEL #8 beam search configs against v2 checkpoints.
...
# Drop any stale beam rows from prior runs before appending
existing = existing[~existing['config'].str.startswith('NOVEL#8')]
combined = pd.concat([existing, new_df], ignore_index=True)
combined.to_csv(out_path, index=False)

print("\n=== Beam search results (3-seed mean) ===")
