...
Seed variance is $\pm$0.19~pp AA, confirming the improvements are architecture-driven.
Applied to wastewater samples without a reference proteome, 6 peptides survive 5\%~FDR;
ESM-2 pseudo-perplexity analysis confirms structural plausibility ($z\!<\!1.0$ vs.\ E.\,coli reference).
\end{abstract}
...
\emph{De novo} sequencing recovers the amino-acid sequence directly from this spectrum
...
Classical database search methods (Mascot, Sequest) are limited by the completeness of the reference
...
\begin{enumerate}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item A \textbf{PeakEncoder with learnable B/Y-ion pair bias}:
        a scalar gate initialized to zero preserves the data-driven optimum while opening a pathway for physics-informed
        attention when the fragment ion signal is reliable.
  ...
  \item A \textbf{real-world wastewater application}: 6 peptides identified at 5\%~FDR from a
        mixed microbial community, validated by ESM-2 structural plausibility scoring.
\end{enumerate}
...
\begin{figure}[t]
  ...
  \caption{
    ...
    running absorbing diffusion with self-conditioning, followed by CFID+SGIR reranking at inference.
  }
  \label{fig:arch}
\end{figure}
...
Crucially, $s$ is \emph{initialized to zero}: the model begins at the data-driven optimum and
...
shorter spectra.
We apply a per-spectrum boolean attention mask that zeroes attention weights to all padding
positions.
This fix is prerequisite to all V2 results; without it, the model exploits inter-spectrum
...
The lesson: mass-aware loss during training is the correct fix, not post-hoc filtering.
...
After filtering: \textbf{472 spectra} for evaluation (80/20 train/val split for training).
...
Table~\ref{tab:main} and Figure~\ref{fig:progression} summarize the full development history
extracted from checkpoint metrics.
...
(1)~Diffusion without PeakEncoder (42--43\%~AA) performs no better than GRU (44\%)---it is the
PeakEncoder, not diffusion alone, that drives the 30~pp jump.
(2)~Self-conditioning then adds +14.6~pp peptide accuracy on top of V1.
(3)~Our final model outperforms InstaNovo on both metrics; the peptide accuracy gap (+26.9~pp)
is especially meaningful as it measures end-to-end correctness.
...
The argmax std for peptide accuracy (6.49~pp) is nearly double that of CFID+SGIR (3.82~pp),
...
At the current confidence level, the model generates candidates whose \emph{composition} is
correct but whose precise fragment masses slightly deviate from the 50~ppm hard threshold.
The right fix is a mass-conditioned training loss, not post-hoc filtering.
...
pseudo-perplexity via simultaneous all-position masking (Kantroo 2024):
\begin{equation}
...
Diffusion V2 predictions have mean PPL $31.03$, within $\Delta = 0.35$ of ground truth ($31.38$).
This validates that the model generates structurally protein-like sequences even for
...
strategies.
The mechanism is clear: absorbing diffusion denoises tokens at each position independently;
...
The absorbing diffusion \emph{without} the PeakEncoder (43.05\%~AA) performs comparably to the
GRU baseline (44.30\%).
It is the PeakEncoder's capacity to localize informative peaks---amplified by the B/Y-ion pair
bias---that enables the 31~pp jump.
This confirms that spectrum encoding is the bottleneck in de novo sequencing, not the decoder.
...
\textbf{(1)}~Test set of 472 spectra is small; confidence intervals would tighten with larger benchmarks.
\textbf{(2)}~No BLAST confirmation of wastewater sequences yet; microbial identity is pending.
\textbf{(3)}~Mass correction is a negative result under current training; a mass-conditioned training loss is the next step.
\textbf{(4)}~B/Y-ion bias gain is modest on clean E.\,coli EV data; its benefit may be larger on nosier wastewater spectra.

%% ─── 8. Related Work ─────────────────────────────────────────────────────────
\section{Related Work}

\textbf{De novo sequencing.}
Casanovo~\citep{casanovo2023} applies an autoregressive Transformer and achieves strong results
on the nine-species benchmark.
InstaNovo~\citep{instanovo2024} adds a diffusion-based refinement step and currently represents
published SOTA.
Our model differs by using pure absorbing diffusion with self-conditioning rather than
autoregressive priming, enabling richer global-context modeling.
...
\bibitem[Abil et~al.(2024)]{instanovo2024}
Abil, Z. et~al.\ (2024).
InstaNovo: Transformers enable accurate, proteome-scale de novo peptide sequencing.
\textit{bioRxiv}.
