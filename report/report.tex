...
\subsection{Mass Constraint at Inference}

Three inference-time strategies for enforcing precursor mass consistency were evaluated; each degrades accuracy, and together they illuminate a common architectural constraint.

\textbf{Post-hoc single-swap correction.} Replacing a residue to bring the predicted mass within
50~ppm of the precursor reduces peptide accuracy by 9.6~pp (59.96\%~$\to$~50.35\%).
The model assigns each residue based on sequence context; substituting it on mass grounds
alone replaces a correct prediction with an incorrect one more often than not.

\textbf{Mass-constrained beam search.} Left-to-right beam search that prunes branches whose
cumulative mass cannot reach the precursor reduces AA recall to 48.2\% ($-28$~pp) and
peptide accuracy to 15.0\% ($-44$~pp).
Absorbing diffusion is a bidirectional model whose logit at each position is conditioned on
the full noisy sequence, not on a committed prefix.
Imposing left-to-right sequential commitment on bidirectional logits invalidates this
conditioning and collapses the predictive distribution.

\textbf{Entropy-adaptive gate.} Per-position masking of mass-infeasible tokens, with tolerance
scaled inversely by positional entropy, reduces AA recall to 12.0\% when applied at the final
argmax step. Combined with CFID, results are statistically indistinguishable from CFID alone
(76.05\% vs.\ 76.02\% AA; 59.53\% vs.\ 59.60\% peptide accuracy), because CFID's iterative
mask-predict refinement already enforces sequence coherence and subsumes whatever mass signal
the gate contributes.

Single-pass mass constraints are incompatible with bidirectional generation. CFID is the
appropriate decoding strategy for this architecture.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../figures/figure8_mass_constraints.png}
  \caption{
    \textbf{Mass constraint strategies vs.\ CFID+SGIR baseline.}
    Post-hoc swap reduces peptide accuracy by 9.6~pp.
    Beam search collapses both metrics (AA recall to 48\%, peptide accuracy to 15\%)
    because left-to-right decoding is incompatible with the bidirectional diffusion model.
    The entropy-adaptive gate alone is similarly catastrophic (12\% AA).
    Gate combined with CFID matches the ungated CFID+SGIR baseline to within 0.4~pp,
    confirming that iterative bidirectional refinement subsumes any mass-feasibility benefit.
  }
  \label{fig:mass_constraints}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{
    \textbf{Inference strategy ablation} (7 strategies, 3 seeds each).
    Error bars show $\pm$std. CFID+SGIR (green) is Pareto-optimal on both metrics.
    Mass-correction variants (red) consistently reduce peptide accuracy by 8--12~pp.
    Red dashed line marks InstaNovo published performance.
  }
  \label{fig:ablation}
\end{figure}
...
\paragraph{Mass constraints at inference.}
Post-hoc correction, beam search, and entropy-adaptive gating each reduce accuracy (by 9.6, 44,
and 48~pp respectively when applied without CFID). The gate combined with CFID matches CFID
alone to within 0.4~pp, confirming that iterative bidirectional refinement subsumes any benefit
from per-position mass feasibility checks. No single-pass constraint improves on CFID+SGIR.
...
\item Inference-time mass constraints (post-hoc swap, beam search, entropy-adaptive gate) all reduce accuracy; the gate combined with CFID matches CFID alone, confirming that bidirectional iterative refinement subsumes single-pass mass feasibility checks.
...
plausibility checks.
Future work should evaluate on larger multi-species benchmarks and confirm the biological
identity of the wastewater peptides through BLAST. Incorporating a hard mass constraint into
the training objective, rather than enforcing it at inference, remains an open direction for
improving mass consistency without compromising sequence accuracy.
